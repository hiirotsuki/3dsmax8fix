...
static void *pSHGetFolderPathA = NULL;
static void *pSHGetFolderPathW = NULL;
...
BOOL WINAPI DllMainCRTStartup(HINSTANCE hinst, DWORD dwReason, LPVOID reserved)
{
	...
	if(dwReason == DLL_PROCESS_ATTACH)
	{
		...
		lstrcatW(sysPath, L"\\shfolder.dll");

		real = LoadLibraryW(sysPath);
		if(!real)
			return FALSE;

		pSHGetFolderPathA = GetProcAddress(real, "SHGetFolderPathA");
		pSHGetFolderPathW = GetProcAddress(real, "SHGetFolderPathW");
...
__attribute__((naked)) __declspec(dllexport) void SHGetFolderPathA(void)
{
...
__attribute__((naked)) __declspec(dllexport) void SHGetFolderPathW(void)
{
	__asm__ __volatile__ (
		"jmp *%0\n"
		:
